...
def get_buah(buah_path):
    print("Loading Nama Buah: {}".format(buah_path))
...
def detect(image_path, model, filename, confidence_cutoff):
    ...
    image = image[:, :, ::-1].copy()
    labels_to_names = {0: 'BuahMatang', 1: 'BuahTidakMatang', -1: 'Sesuatu'}
    ...
    detect_result = {"BuahMatang": 0, "BuahTidakMatang": 0, "Sesuatu":0}

    cutoff_BuahMatang = confidence_cutoff['BuahMatang']
    cutoff_BuahTidakMatang = confidence_cutoff['BuahTidakMatang']
    ...
    for box, score, label in zip(boxes[0], scores[0], labels[0]):
        ...
        if label == 0 and score < cutoff_BuahMatang:
            continue
        elif label == 1 and score < cutoff_BuahTidakMatang:
            continue
        i += 1
